...
TOOL_TIMEOUT = int(os.environ.get("AI4MATH_TOOL_TIMEOUT", "120"))
...
def _call_tool_in_process(name: str, arguments: dict[str, Any], timeout: int) -> str:
    result_box: list[str] = []
...
def _call_tool_in_subprocess(name: str, arguments: dict[str, Any], timeout: int) -> str:
    from ai4math.tools.sage_subprocess import run_sage_tool_subprocess

    return run_sage_tool_subprocess(name=name, arguments=arguments, timeout=timeout, cwd=os.getcwd())
...
class ToolRegistry:
    ...
    def call_tool(name: str, arguments: dict[str, Any], timeout: int | None = None) -> str:
        ...
        if entry["category"].startswith("sage"):
            return _call_tool_in_subprocess(name, arguments, deadline)
        return _call_tool_in_process(name, arguments, deadline)
